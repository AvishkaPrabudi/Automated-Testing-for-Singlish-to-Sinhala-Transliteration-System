...
def _find_column_index(header_values: list, requested_name: str | None, candidates: list[str]) -> int | None:
    ...
    def partial_match(name: str) -> int | None:
        n = _normalize_header(name)
        if not n:
            return None
        for i, v in indexed:
    ...
    if requested_name:
        ...
        if found:
            return found
        found = partial_match(requested_name)
        if found:
            return found
...
def _wait_for_output_value(page, output_locator, previous_output: str, timeout_ms: int, min_wait_ms: int = 1000) -> str:
    deadline = time.time() + (max(1, timeout_ms) / 1000)
    minimum_ready_at = time.time() + (max(0, min_wait_ms) / 1000)
    last_value = previous_output.strip()

    while time.time() < deadline:
        try:
            current = _read_output(True, output_locator).strip()
            if current:
                if current != last_value or time.time() >= minimum_ready_at:
                    return current
        except Exception:
            pass
    ...
    try:
        return _read_output(True, output_locator).strip()
    except Exception:
        return ""
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                actual_output = _wait_for_output_value(
                    page,
                    output_locator,
                    prev_output,
                    int(args.timeout_ms),
                    min_wait_ms=max(1000, int(args.wait_ms))
                )
